(function(window, document, $, undefined){
                                  ...
                                  placeholder_url, max_imagesize, onImageUpload, onKeyEnter )
    {
        ...
        var content_insertlink = function(wysiwygeditor, $modify_link)
        {
            ...
            var $inputName = $('<input type="text" value="" placeholder="请输入网址名称..." style="display: block;width: 23em;margin: 5px 0px;">').addClass('wysiwyg-input');
            var $inputurl = $('<input type="text" value="' + ($modify_link ? $modify_link.attr('href') : '') + '" placeholder="请输入网址...." />').addClass('wysiwyg-input')
            ...
            if( window.File && window.FileReader && window.FileList )
            {
                ...
                var loadImageFromFile = function( file )
                {
                    ...
                    $.ajaxFileUpload({
                        ...
                        error: function() {
                            toastrTips("异常，图片上传失败！","error");
                        }
                    });


                };
                $fileuploader = $('<input type="file" name="articleImage" id="articleImage" />')
                                    .attr('draggable','true')
